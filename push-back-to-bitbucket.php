...
include __DIR__ . '/bitbucket-utils.php';
...
print "Enter push-back-to-bitbucket. repository root is $repositoryRoot, docRoot is $docRoot\n";
...
$bitbucketSecretsFile = "$privateFiles/bitbucket-secrets.json";
if (!file_exists($privateFiles)) {
  print "Could not find $bitbucketSecretsFile\n";
  return;
}
$bitbucketSecretsContents = file_get_contents($bitbucketSecretsFile);
$bitbucketSecrets = json_decode($bitbucketSecretsContents, true);
if (empty($bitbucketSecrets)) {
  print "No data in BitBucket secrets\n";
  return;
}
$token = bitbucket_get_token($bitbucketSecrets);
...
if (!empty($token)) {
  ...
  $upstreamRepoWithCredentials = str_replace('https://', "https://x-token-auth:$token@", $upstreamRepoWithCredentials);
}
